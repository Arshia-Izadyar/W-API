...
func (l *zeroLogger) Error(err error, cat Category, sub SubCategory, msg string, extra map[ExtraKey]interface{}) {
	l.logger.Error().
		Str("Category", string(cat)).
		Str("SubCategory", string(sub)).
		Fields(mapToZeroParams(extra)).
...
func (l *zeroLogger) Errorf(err error, template string, args ...interface{}) {
	l.logger.Error().Msgf(template, args)
}

func (l *zeroLogger) Fatal(err error, cat Category, sub SubCategory, msg string, extra map[ExtraKey]interface{}) {
	l.logger.Fatal().
		Str("Category", string(cat)).
		Str("SubCategory", string(sub)).
		Fields(mapToZeroParams(extra)).
...
func (l *zeroLogger) Fatalf(err error, template string, args ...interface{}) {
